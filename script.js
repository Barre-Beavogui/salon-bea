const SALON_WHATSAPP = '224625385342';
...
const defaultPosts = [
  ...
  },
  {
    id: 'p5',
    type: 'video',
    title: 'Formation tresses',
    description: 'Extrait d’une séance pratique.',
    url: ''
  },
  {
    id: 'p6',
    ...
    description: 'Large choix de textures et couleurs.',
    url: ''
  }
];
...
const storage = {
  ...
  set(key, value) {
    try {
      ...
      // Local storage may be disabled; ignore.
    }
  }
};

function initPosts() {
  posts = storage.get('bea_posts', null);
  if (!Array.isArray(posts) || posts.length === 0) {
    posts = defaultPosts;
    storage.set('bea_posts', posts);
  }
  renderPosts();
}

function initAppointments() {
  appointments = storage.get('bea_appointments', []);
...
function renderAppointments() {
  ...
  appointments.forEach((appointment) => {
    ...
    if (appointment.phone) {
      ...
      wa.href = buildClientWhatsApp(appointment);
      wa.target = '_blank';
      wa.rel = 'noopener';
      ...
      call.href = `tel:${normalizePhone(appointment.phone)}`;
    ...
    if (appointment.email) {
      ...
      mail.href = buildClientEmail(appointment);
...
function buildClientWhatsApp(appointment) {
  const number = normalizePhoneDigits(appointment.phone);
  const message = `Bonjour ${appointment.name}, votre rendez-vous ${appointment.service} est bien noté pour le ${appointment.date} à ${appointment.time}.`;
  return `https://wa.me/${number}?text=${encodeURIComponent(message)}`;
}

function buildClientEmail(appointment) {
  const subject = `Votre rendez-vous ${appointment.service}`;
  const body = `Bonjour ${appointment.name},\n\nVotre rendez-vous est bien noté pour le ${appointment.date} à ${appointment.time}.\n\nMerci.`;
  return `mailto:${appointment.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}

function saveAppointments() {
  storage.set('bea_appointments', appointments);
}
